...
instance Show a => Show (HsTyCon a) where
  show (HsTC a) = show a
...
instance Show a => Show (HsDataCon a) where
  show = show . unHsDC
...
bugDataConSym :: Sym
bugDataConSym = mkSym "Bug"

bugDataConName :: Name
bugDataConName = mkName bugDataConSym bugDataConUnique

psBugDataCon :: PsDataCon
psBugDataCon = HsDC bugDataConSym

rnBugDataCon :: RnDataCon
rnBugDataCon = HsDC bugDataConName

bugDataConInfo :: HsDataConInfo
bugDataConInfo = HsDCInfo rnBugDataCon [] rnTestResultTyCon [] fcBugDataCon

isBugDataCon :: Symable a => HsDataCon a -> Bool
isBugDataCon (HsDC a) = symOf a == bugDataConSym

isHsBugTm :: Symable a => Term a -> Bool
isHsBugTm (TmCon _tId dc) = isBugDataCon dc
isHsBugTm  _other_tm      = False

validDataConSym :: Sym
validDataConSym = mkSym "Valid"

validDataConName :: Name
validDataConName = mkName validDataConSym validDataConUnique

psValidDataCon :: PsDataCon
psValidDataCon = HsDC validDataConSym

rnValidDataCon :: RnDataCon
rnValidDataCon = HsDC validDataConName

validDataConInfo :: HsDataConInfo
validDataConInfo = HsDCInfo rnValidDataCon [] rnTestResultTyCon [] fcValidDataCon

isValidDataCon :: Symable a => HsDataCon a -> Bool
isValidDataCon (HsDC a) = symOf a == validDataConSym

isHsValidTm :: Symable a => Term a -> Bool
isHsValidTm (TmCon _tId dc) = isValidDataCon dc
isHsValidTm  _other_tm      = False
...
data Term a = TmVar TermId (HsTmVar a)                   -- ^ Term variable
            | TmCon TermId (HsDataCon a)                 -- ^ Data constructor
            | TmAbs TermId (HsTmVar a) (Term a)          -- ^ Lambda x . Term
            | TmApp TermId (Term a) (Term a)             -- ^ Term application
            | TmLet TermId (HsTmVar a) (Term a) (Term a) -- ^ Letrec var = term in term
            | TmCase TermId (Term a) [HsAlt a]           -- ^ case e of { ... }

instance Show a => Show (Term a) where
  show (TmVar _ x)         = show x
  show (TmCon _ dc)        = show dc
  show (TmAbs _ x tm)      = "(\\" ++ show x ++ ". " ++ show tm ++ ")"
  show (TmApp _ tm1 tm2)   = "(" ++ show tm1 ++ " " ++ show tm2 ++ ")"
  show (TmLet _ x tm1 tm2) = "(let " ++ show x ++ " = " ++ show tm1 ++ " in " ++ show tm2 ++ ")"
  show (TmCase _ tm alts)  = "(case " ++ show tm ++ " of { " ++ concatMap ((++ "; ") . show) alts ++ " }" ++ ")"

instance HasTermId (Term a) where
  termId (TmVar tId _)     = tId
  termId (TmCon tId _)     = tId
  termId (TmAbs tId _ _)   = tId
  termId (TmApp tId _ _)   = tId
  termId (TmLet tId _ _ _) = tId
  termId (TmCase tId _ _)  = tId
  updateTermId tId (TmVar _ x)         = TmVar tId x
  updateTermId tId (TmCon _ con)       = TmCon tId con
  updateTermId tId (TmAbs _ x tm)      = TmAbs tId x tm
  updateTermId tId (TmApp _ tm1 tm2)   = TmApp tId tm1 tm2
  updateTermId tId (TmLet _ x tm1 tm2) = TmLet tId x tm1 tm2
  updateTermId tId (TmCase _ tm alts)  = TmCase tId tm alts
...
instance Show a => Show (HsAlt a) where
  show (HsAlt pat tm) = show pat ++ " -> " ++ show tm

instance Show a => Show (HsPat a) where
  show (HsPat con xs) = show con ++ concatMap ((" " ++) . show) xs
...
instance Show a => Show (MonoTy a) where
  show (TyCon con    ) = "(" ++ show con ++ ")"
  show (TyApp ty1 ty2) = "(" ++ show ty1 ++ " " ++ show ty2 ++ ")"
  show (TyVar var    ) = show var
...
testResultTyConSym :: Sym
testResultTyConSym = mkSym "TestResult"

testResultTyConName :: Name
testResultTyConName = mkName testResultTyConSym testResultTyConUnique

psTestResultTyCon :: PsTyCon
psTestResultTyCon = HsTC testResultTyConSym

rnTestResultTyCon :: RnTyCon
rnTestResultTyCon = HsTC testResultTyConName

testResultTyConInfo :: HsTyConInfo
testResultTyConInfo = HsTCInfo rnTestResultTyCon [] fcTestResultTyCon

isTestResultTyCon :: Symable a => HsTyCon a -> Bool
isTestResultTyCon (HsTC a) = symOf a == testResultTyConSym

isHsTestResultTy :: Symable a => MonoTy a -> Bool
isHsTestResultTy (TyCon tc) = isTestResultTyCon tc
isHsTestResultTy _other_ty  = False
...
instance Show (Program a) where
    show (PgmExp tm) = show tm
    show (PgmCls _ p) = show p
    show (PgmInst _ p) = show p
    show (PgmData _ p) = show p
-}
-- | Class declaration
...
instance Semigroup FullTheory where
  FT ts1 ti1 tl1 <> FT ts2 ti2 tl2 = FT (ts1 <> ts2) (ti1 <> ti2) (tl1 <> tl2)

instance Monoid FullTheory where
  mempty = FT mempty mempty mempty
...
-- ------------------------------------------------------------------------------
instance Eq a => ContainsFreeTmVars (Term a) (HsTmVar a) where
  ftmvsOf (TmVar _ v)         = [v]
  ftmvsOf (TmCon _ _)         = []
  ftmvsOf (TmAbs _ v tm)      = ftmvsOf tm \\ [v]
  ftmvsOf (TmApp _ tm1 tm2)   = nub $ ftmvsOf tm1 ++ ftmvsOf tm2
  ftmvsOf (TmLet _ v tm1 tm2) = nub $ (ftmvsOf tm1 ++ ftmvsOf tm2) \\ [v]
  ftmvsOf (TmCase _ tm alts)  = nub $ ftmvsOf tm ++ concat (map ftmvsOf alts)

instance Eq a => ContainsFreeTmVars (HsAlt a) (HsTmVar a) where
  ftmvsOf (HsAlt pat tm) = ftmvsOf tm \\ ftmvsOf pat

instance ContainsFreeTmVars (HsPat a) (HsTmVar a) where
  ftmvsOf (HsPat _ vs) = vs
...
instance (Symable a, PrettyPrint a) => PrettyPrint (Term a) where
  ppr (TmVar _id v)          = ppr v
  ppr (TmCon _id dc)         = ppr dc
  ppr (TmAbs _id v tm)       = backslash <> ppr v <> dot <+> ppr tm
  ppr (TmApp _id tm1 tm2)
    | TmApp {} <- tm1        = ppr tm1    <+> pprPar tm2
    | otherwise              = pprPar tm1 <+> pprPar tm2
  ppr (TmLet _id v tm1 tm2)  = colorDoc yellow (text "let") <+> ppr v <+> equals <+> ppr tm1
                        $$ colorDoc yellow (text "in")  <+> ppr tm2
  ppr (TmCase _id scr alts)  = hang (text "case" <+> ppr scr <+> text "of") 2 (vcat $ map ppr alts)
