...
module Frontend.HsTypeChecker (hsElaborate, hsElaborateTesters) where
...
instance Semigroup TcEnv where
  TcEnv cls1 dc1 tc1 <> TcEnv cls2 dc2 tc2 = TcEnv (cls1 <> cls2) (dc1 <> dc2) (tc1 <> tc2)
...
instance MonadUnique SolveM where
  getUniqueSupplyM = liftSolveM $ lift getUniqueSupplyM
...
-- |
data TypeInfData = TyInfD ConstraintStore (TermTable RnMonoTy)

instance Semigroup TypeInfData where
  (<>) (TyInfD cs1 table1) (TyInfD cs2 table2)
    = TyInfD (cs1 <> cs2) (table1 <> table2)

instance Monoid TypeInfData where
  mempty = TyInfD mempty mempty

onCs :: (ConstraintStore -> ConstraintStore) -> TypeInfData -> TypeInfData
onCs f (TyInfD cs table) = TyInfD (f cs) table

-- | Type inference generation monad
newtype GenM a = GenM (StateT TypeInfData TcM a)
  deriving ( Functor, Applicative, Monad
           , MonadState TypeInfData, MonadReader TcCtx, MonadError String, GenTmIdMonad RnTerm )
...
instance MonadUnique GenM where
  getUniqueSupplyM = liftGenM $ lift getUniqueSupplyM

-- | Get out of the constraint generation monad
runGenM :: GenM a -> TcM (a, EqCs, AnnClsCs, TermTable RnMonoTy)
runGenM (GenM m) = do
    (v, TyInfD (CS eqs ccs) table) <- runStateT m mempty
    return (v, eqs, ccs, table)

-- | Lift TcM into GenM
liftGenM :: TcM a -> GenM a
liftGenM m = GenM $ StateT ((<$> m) . flip (,))

-- | Add some equality constraints in the constraint store
storeEqCs :: EqCs -> GenM ()
storeEqCs cs = modify $ onCs (\(CS eqs ccs) -> CS (cs ++ eqs) ccs)

-- | Add some (variable-annotated) class constraints in the constraint store
storeAnnCts :: AnnClsCs -> GenM ()
storeAnnCts cs = modify $ onCs (\(CS eqs ccs) -> CS eqs (mappend ccs cs))

-- |
storeTypeEntry :: TermId -> RnMonoTy -> GenM ()
storeTypeEntry tmId ty = modify (\(TyInfD cs table) -> TyInfD cs (M.insert tmId ty table))
...
elabTerm' (TmApp _tmId tm1 tm2)   = elabTmApp tm1 tm2
elabTerm' (TmAbs _tmId x tm)      = elabTmAbs x tm
elabTerm' (TmVar _tmId x)         = elabTmVar x
elabTerm' (TmCon _tmId dc)        = liftGenM (elabTmCon dc)
elabTerm' (TmLet _tmId x tm1 tm2) = elabTmLet x tm1 tm2
elabTerm' (TmCase _tmId scr alts) = elabTmCase scr alts

elabTerm :: RnTerm -> GenM (RnMonoTy, FcTerm)
elabTerm tm = do
  tId <- genTerm tm
  (ty, fcTm) <- elabTerm' tm
  storeTypeEntry (termId tm) ty
  return (ty, updateTermId tId fcTm)
...
elabTmApp tm1 tm2 = do
  ...
  return (a, FcTmApp invalidTmId fc_tm1 fc_tm2)
...
elabTmAbs x tm = do
  ...
  let result = FcTmAbs invalidTmId (rnTmVarToFcTmVar x) (rnTyVarToFcType tv) fc_tm
...
elabTmVar x = do
  ...
  let fc_ds = map (FcTmVar invalidTmId) ds -- System F representation
...
elabTmLet x tm1 tm2 = do
  ...
  let fc_tm = FcTmLet invalidTmId (rnTmVarToFcTmVar x) (rnTyVarToFcType tv) fc_tm1 fc_tm2
  return (ty2, fc_tm)
...
elabTmCon dc = do
  ...
  let fc_tm = fcTmTyApp (FcTmDataCon invalidTmId fc_dc)            -- System F term
                        (rnTyVarsToFcTypes bs)
...
elabTmCase scr alts = do
  ...
  return (rhs_ty, FcTmCase invalidTmId fc_scr fc_alts)
...
rnTmVarToFcTerm = FcTmVar invalidTmId . rnTmVarToFcTmVar
...
--   b) return the new wanted (class) constraints, as well as the System F term substitution
...
leftEntails untch (d_g :| ctr_g) (d_w :| cls_ct_w) = do
  ...
  matchClsCs untch (d_g :| cls_ct_g) (d_w :| cls_ct_w) >>= \case
    ...
    Just (ty_subst, _) -> do
      ...
      ev_subst_tm              <- constructEvFcTerm ty_subst (FcTmVar invalidTmId d_g) as d_res
  ...
  where
    ...
    constructEvFcTerm  ty_subst fc_tm []     (d:ds) =
      constructEvFcTerm ty_subst (FcTmApp invalidTmId fc_tm (FcTmVar invalidTmId d)) [] ds
    constructEvFcTerm  ty_subst fc_tm (a:as) ds     =
      elabMonoTy (substInMonoTy ty_subst (TyVar (labelOf a))) >>= \subst_fc_ty ->
      constructEvFcTerm ty_subst (FcTmTyApp invalidTmId fc_tm subst_fc_ty) as ds
...
matchClsCs untch (d1 :| ClsCt cls1 ty1) (d2 :| ClsCt cls2 ty2)
  ...
  = return $ Just (ty_subst, d2 |-> FcTmVar invalidTmId d1)
...
elabClsDecl (ClsD rn_cs cls (a :| _) method method_ty) = do
  ...
  (sc_schemes, sc_decls) <- fmap unzip $ forM (zip [0..] rn_cs) $ \(i,sc_ct) -> do
    ...
    let fc_tm = FcTmTyAbs invalidTmId (rnTyVarToFcTyVar a)
              $ FcTmAbs invalidTmId da fc_cls_head
              $ FcTmCase invalidTmId (FcTmVar invalidTmId da)
                                   [FcAlt (FcConPat dc xs) (FcTmVar invalidTmId (xs !! i))]
...
elabMethodSig method a cls sigma = do
  ...
  let fc_method_rhs = fcTmTyAbs (map rnTyVarToFcTyVar bs) $
                        fcTmAbs dbinds $
                          FcTmCase invalidTmId
                                   (FcTmVar invalidTmId (head ds))
                                   [FcAlt (FcConPat dc xs)
                                          (fcDictApp (fcTmTyApp (FcTmVar invalidTmId (last xs))
                                                                (tail rn_bs))
                                                     (tail ds))]
...
elabFuncDecl :: FullTheory -> RnFuncDecl
             -> TcM (FcValBind, TcCtx, TermTable RnMonoTy)
elabFuncDecl theory (FuncD func func_ty func_tm) = do
  ...
  (fc_tm, tyTable) <- setTcCtxTmM ty_ctx (elabTermWithSig [] theory func_tm func_ty)

  -- Resulting binding
  let fc_val_bind = FcValBind (rnTmVarToFcTmVar func) fc_func_ty fc_tm

  return (fc_val_bind, ty_ctx, tyTable)
...
elabInsDecl :: FullTheory -> RnInsDecl
            -> TcM (FcValBind, FullTheory, TermTable RnMonoTy)
elabInsDecl theory (InsD ins_ctx cls typat method method_tm) = do
  ...
  fc_super_tms <- do
    ...
    (ds, super_cs) <- substVar a (hsTyPatToMonoTy typat) <$> lookupClsSuper cls >>= annotateClsCs

    ev_subst <- entailTcM (map labelOf bs) (ftToProgramTheory local_theory) super_cs

    return (map (substFcTmInTm ev_subst . FcTmVar invalidTmId) ds)
  ...
  return (fc_val_bind, ext_theory, tyTable)
  where
    bs      = ftyvsOf typat
...
elabTermWithSig :: [RnTyVar] -> FullTheory -> RnTerm -> RnPolyTy
                -> TcM (FcTerm, TermTable RnMonoTy)
elabTermWithSig untch theory tm poly_ty = do

  -- Infer the type of the expression and the wanted constraints
  ((mono_ty, fc_tm), wanted_eqs, wanted_ccs, tyTable) <- runGenM (elabTerm tm)
  ...
  partial_result <- closeRemainingTyVars untouchables
                  $ substFcTmInTm ev_subst
                  $ substFcTyInTm fc_subst fc_tm
  
  return ( updateTermId tId'
         $ fcTmTyAbs fc_as
         $ fcTmAbs dbinds
         $ partial_result
         , fmap (substInMonoTy ty_subst) tyTable )
  where
    (as,cs,ty) = destructPolyTy poly_ty
...
-- |
closeRemainingTyVars :: [RnTyVar] -> FcTerm -> TcM FcTerm
closeRemainingTyVars untch tm = do
  let tvs = ftyvsOf tm \\ map rnTyVarToFcTyVar untch
  tys <- mapM (((\v -> FcTyAbs v (FcTyVar v)) <$>) . freshFcTyVar . kindOf) tvs
  return $ fcTmTyApp (fcTmTyAbs tvs tm) tys

-- * Type Inference With Constraint Simplification
-- ------------------------------------------------------------------------------

elabTermSimpl :: ProgramTheory -> RnTerm
              -> TcM (RnPolyTy, FcTerm, TermTable RnMonoTy)
...
-- entailDetTcM :: [RnTyVar] -> ProgramTheory -> ProgramTheory -> TcM (ProgramTheory, FcTmSubst)
  ...
  let full_fc_tm = updateTermId tId'
                 $ fcTmTyAbs fc_as
                 $ fcTmAbs dbinds
                 $ substFcTmInTm ev_subst
                 $ refined_fc_tm
  
  let newTyTable = substInMonoTy ty_subst <$> tyTable
  
  return (gen_ty, full_fc_tm, newTyTable)
...
elabProgram :: FullTheory -> RnProgram
            -> TcM ( FcProgram          {- Elaborated program       -}
                   ...
                   , TermTable RnMonoTy {- Types of subterms        -}
                   , TcCtx )            {- Context of main term     -}

-- Elaborate the program expression
elabProgram theory (PgmExp tm) = do
  (ty, fc_tm, tyTable) <- elabTermSimpl (ftDropSuper theory) tm
  ctx <- ask
  return (FcPgmTerm fc_tm, ty, theory, tyTable, ctx)
...
elabProgram theory (PgmCls cls_decl pgm) = do
  ...
  (fc_pgm, ty, final_theory, tyTable, ctx) <-
    setTcCtxTmM ext_ty_env (elabProgram (theory `ftExtendSuper` ext_theory) pgm)
  let fc_program = FcPgmDataDecl fc_data_decl
                                 (FcPgmValDecl fc_val_bind
                                               (foldl (flip FcPgmValDecl) fc_pgm fc_sc_proj))
  return (fc_program, ty, final_theory, tyTable, ctx)

-- | Elaborate a class instance
elabProgram theory (PgmInst ins_decl pgm) = do
  (fc_val_bind, ext_theory, tyTable1) <- elabInsDecl theory ins_decl 
  (fc_pgm, ty, final_theory, tyTable2, ctx) <- elabProgram ext_theory pgm
  let fc_program = FcPgmValDecl fc_val_bind fc_pgm
  return (fc_program, ty, final_theory, tyTable1 <> tyTable2, ctx)

-- Elaborate a datatype declaration
elabProgram theory (PgmData data_decl pgm) = do
  fc_data_decl <- elabDataDecl data_decl
  (fc_pgm, ty, final_theory, tyTable, ctx) <- elabProgram theory pgm
...
elabProgram theory (PgmFunc func_decl pgm) = do
  (fc_val_bind, ext_ty_env, tyTable1) <- elabFuncDecl theory func_decl
  (fc_pgm, ty, final_theory, tyTable2, ctx) <- setTcCtxTmM ext_ty_env $ elabProgram theory pgm
  let fc_program = FcPgmValDecl fc_val_bind fc_pgm
  return (fc_program, ty, final_theory, tyTable1 <> tyTable2, ctx)
...
type ElaborationOuter a = ( Either String
                                   (((a, GenTab RnTerm), UniqueSupply), TcEnv)
                          , Trace )

type ElaborationInner a = ( ( a
                            , RnPolyTy
                            , FullTheory
                            , TermTable RnMonoTy
                            , TcCtx )
...
type Elaboration a = ElaborationOuter (ElaborationInner a)

unpackElab :: TcEnv -> TcCtx -> UniqueSupply -> TermId -> TcM a -> ElaborationOuter a
unpackElab tc_init_gbl_env tc_init_ctx us tId = runWriter
                                              . runExceptT
                                              . flip runStateT tc_init_gbl_env
                                              . flip runReaderT tc_init_ctx
                                              . flip runUniqueSupplyT us
                                              . flip runGenTmIdT tId

elabMonad :: RnEnv -> FullTheory -> RnProgram -> TcM (ElaborationInner FcProgram)
elabMonad rn_gbl_env tc_init_theory pgm = do
  buildInitTcEnv pgm rn_gbl_env -- Create the actual global environment
  result <- elabProgram tc_init_theory pgm
  assocs <- buildInitFcAssocs
  return (result, assocs)

hsElaborate :: RnEnv -> UniqueSupply -> TermId -> RnProgram -> Elaboration FcProgram
hsElaborate rn_gbl_env us tId pgm = unpackElab tc_init_gbl_env tc_init_ctx us tId
                                  $ elabMonad rn_gbl_env tc_init_theory pgm
  where
    tc_init_theory  = mempty
    tc_init_ctx     = mempty
    tc_init_gbl_env = mempty

hsElaborateTesters :: FullTheory -> TcCtx -> TcEnv -> UniqueSupply -> TermId -> [RnTerm]
                   -> ElaborationOuter ([(RnPolyTy, FcTerm)], Tests)
hsElaborateTesters theory
                   tc_gbl_ctx
                   tc_gbl_env
                   us
                   tId
                   tms        = unpackElab tc_init_gbl_env tc_init_ctx us tId
                              $ do
                                testers <- mapM elabTester tms
                                us' <- getUniqueSupplyM
                                return (testers, testsFrom us' $ zip tms testers)
  where
    testsFrom :: UniqueSupply -> [(RnTerm, (RnPolyTy, FcTerm))] -> Tests
    testsFrom us' = genTests tc_init_theory tc_init_ctx tc_gbl_env us' tId
    
    elabTester :: RnTerm -> TcM (RnPolyTy, FcTerm)
    elabTester tester = do
      (ty, tm, _) <- elabTermSimpl (ftDropSuper tc_init_theory) tester
      checkType ty
      return (ty, tm)
      
    checkType :: RnPolyTy -> TcM ()
    checkType ty = maybe (throwErrorM (text "Invalid tester type")) return $ do
      let (_, _, ty') = destructPolyTy ty
      (_, res) <- isHsArrowTy ty'
      if isHsTestResultTy res
      then Just ()
      else Nothing
    
    tc_init_theory  = theory
    tc_init_ctx     = tc_gbl_ctx
    tc_init_gbl_env = tc_env_testers <> tc_gbl_env

genTests :: FullTheory -> TcCtx -> TcEnv -> UniqueSupply -> TermId -> [(RnTerm, (RnPolyTy, FcTerm))]
         -> Tests
genTests theory tc_gbl_ctx tc_gbl_env us tId candidates tmDesc =
  map (\(rnCand, elabCand) -> fmap ((,) rnCand) $ unpackTest $ genTest elabCand) candidates
  where
    genTest :: (RnPolyTy, FcTerm) -> ElaborationOuter FcTermInterp
    genTest (testerTy, testerTm) = unpackElab tc_gbl_env' tc_gbl_ctx us tId $ do
      case tmDesc of
        Left (testeeTy,testeeTm) -> do
          let (bs,cs,ty) = destructPolyTy testerTy
          ty_subst <- unify [] [TyApp (TyApp (TyCon rnArrowTyCon) testeeTy) (TyCon rnTestResultTyCon) :~: ty]
          
          (ds,ann_cs) <- annotateClsCs cs
          let wanted = substInSimpleProgramTheory ty_subst ann_cs
          ev_subst <- entailTcM [] progTheory wanted
          let fc_ds = map (fcTermToInterp . applySubst ev_subst . FcTmVar invalidTmId) ds
          fc_bs <- mapM (elabMonoTy . applySubst ty_subst . TyVar . labelOf) bs
          return $ FcTmApp invalidTmId
                           (fcTmApp (fcTmTyApp (fcTermToInterp testerTm) fc_bs) fc_ds)
                           testeeTm
        Right testeeTm           -> do
          testerVar <- freshRnTmVar
          let testRnTm = TmApp invalidTmId (TmVar invalidTmId testerVar) testeeTm
          (_, testFcTm, _) <- local ( flip (flip extendCtxTm testerVar) testerTy )
                                    (elabTermSimpl progTheory testRnTm)
          return $ substVar (rnTmVarToFcTmVar testerVar) (fcTermToInterp testerTm)
                 $ {-fillTyAbs $ -} fcTermToInterp testFcTm
    
    fillTyAbs :: FcTermInterp -> FcTermInterp
    fillTyAbs (FcTmTyAbs tId a tm) = FcTmTyApp invalidTmId
                                               (FcTmTyAbs tId a (fillTyAbs tm))
                                               (FcTyCon fcTestResultTyCon)
    fillTyAbs tm                   = tm
    
    unpackTest :: ElaborationOuter FcTermInterp -> Maybe FcTermInterp
    unpackTest (Left  e                 , _) = Nothing
    unpackTest (Right (((result,_),_),_), _) = Just result
    
    tc_gbl_env' = tc_env_testers <> tc_gbl_env
    
    progTheory = ftToProgramTheory theory

tc_env_tester_types :: TcEnv
tc_env_tester_types = TcEnv { tc_env_cls_info = mempty
                            , tc_env_dc_info  = mempty
                            , tc_env_tc_info  = extendAssocList rnTestResultTyCon
                                                                testResultTyConInfo
                                              $ mempty }

tc_env_testers :: TcEnv
tc_env_testers =
  tc_env_tester_types { tc_env_dc_info  = extendAssocList rnBugDataCon bugDataConInfo
                                        $ extendAssocList rnValidDataCon validDataConInfo
                                        $ mempty }
