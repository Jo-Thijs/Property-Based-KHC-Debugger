...
module Frontend.HsRenamer (RnEnv(..), hsRename, hsRenameTesters) where
...
rnTerm (TmVar termId x)          = TmVar termId <$> lookupTmVarM x
rnTerm (TmCon termId dc)         = TmCon termId <$> lookupDataCon dc
rnTerm (TmAbs termId psx pstm)   = do
  rnx  <- rnTmVar psx
  rntm <- extendCtxTmM psx rnx (rnTerm pstm)
  return (TmAbs termId rnx rntm)
rnTerm (TmApp termId tm1 tm2)    = TmApp termId <$> rnTerm tm1 <*> rnTerm tm2
rnTerm (TmLet termId x tm1 tm2)  = do
  rnx   <- rnTmVar x
  rntm1 <- extendCtxTmM x rnx (rnTerm tm1)
  rntm2 <- extendCtxTmM x rnx (rnTerm tm2)
  return (TmLet termId rnx rntm1 rntm2)
rnTerm (TmCase termId scr alts)  = TmCase termId <$> rnTerm scr <*> mapM rnAlt alts
...
rnFuncDecl (FuncD func func_ty func_tm) = do
  ...
  -- Get the current typing environment (so that we can extend it with the function binding)
  rn_ctx <- ask
...
hsRenameTesters :: UniqueSupply -> RnCtx -> RnEnv -> [PsTerm]
              -> (Either String ([RnTerm], UniqueSupply), Trace)
hsRenameTesters us rn_ctx rn_env tms = runWriter
                                     $ runExceptT
                                     $ flip evalStateT rn_init_gbl_env
                                     $ flip runReaderT rn_init_ctx
                                     $ flip runUniqueSupplyT us
                                     $ mapM rnTerm tms
  where
    rn_init_ctx     = rn_ctx
    rn_init_gbl_env = rn_env { rn_env_dc_info = rn_env_dc_info_ext (rn_env_dc_info rn_env)
                             , rn_env_tc_info = rn_env_tc_info_ext (rn_env_tc_info rn_env)
                             }
    rn_env_dc_info_ext = extendAssocList psBugDataCon bugDataConInfo
                       . extendAssocList psValidDataCon validDataConInfo
    rn_env_tc_info_ext = extendAssocList psArrowTyCon arrowTyConInfo
                       . extendAssocList psTestResultTyCon testResultTyConInfo
