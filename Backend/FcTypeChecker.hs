...
tcTerm (FcTmVar _ x) = lookupTmVarM x
tcTerm (FcTmApp _ tm1 tm2)  = do
  ty1 <- tcTerm tm1
  ty2 <- tcTerm tm2
...
tcTerm (FcTmDataCon _ dc) = mkDataConTy <$> lookupDataConTyM dc
tcTerm (FcTmLet _ x ty tm1 tm2) = do
...
tcAlts :: FcType -> [FcAlternative] -> FcM FcType
tcAlts scr_ty alts
...
tcAlt :: FcType -> FcAlternative -> FcM FcType
