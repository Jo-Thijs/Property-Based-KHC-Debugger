...
, rnTmVarToFcTmVar, rnTyVarToFcTyVar
  -- * Do the inverse
, fcTmVarToRnTmVar, fcTyVarToRnTyVar
...
instance Show Name where
  show (MkName sym unique) = show sym ++ "<" ++ show unique ++ ">"
...
instance Show a => Show (HsTmVar a) where
  show = show . hstmvar_name
...
instance Ord a => Ord (HsTyVar a) where
  compare (PsTyVar a  ) (PsTyVar b  ) = compare a b
  compare (RnTyVar a _) (RnTyVar b _) = compare a b
...
instance Show FcTmVar where
  show = show . fctmvar_name

instance Show FcTyVar where
  show = show . fctyvar_name
...
-- | Convert a System F term variable to a source renamed term variable
fcTmVarToRnTmVar :: FcTmVar -> HsTmVar Name
fcTmVarToRnTmVar (FcTmVar name) = HsTmVar name

-- | Convert a System F type variable to a source renamed type variable
...
-- | Create a fresh renamed type variable
