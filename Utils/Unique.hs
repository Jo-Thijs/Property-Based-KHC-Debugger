...
, bugDataConUnique, validDataConUnique, testResultTyConUnique
, templateTyConUnique
) where
...
testResultTyConUnique :: Unique
testResultTyConUnique = MkU 'c' 0

templateTyConUnique :: Unique
templateTyConUnique = MkU 'c' 2

-- ----------------------------------------------------------------------------
--                           Debug DataCon Unique
-- ----------------------------------------------------------------------------

bugDataConUnique :: Unique
bugDataConUnique = MkU 'c' 1

validDataConUnique :: Unique
validDataConUnique = MkU 'c' 2
...
-- | Get the Unique of a UniqueSupply
takeUniqueFromSupply :: Monad m => UniqueSupply -> m (Unique, UniqueSupply)
takeUniqueFromSupply (Node u us1 _) = return (u, us1)
...
splitUniqueSupply :: Monad m => UniqueSupply -> m (UniqueSupply, UniqueSupply)
splitUniqueSupply (Node _ us1 us2) = return (us1, us2)
...
type UniqueSupplyM = UniqueSupplyT Identity

-- | Run a UniqueSupplyM computation
runUniqueSupplyM :: UniqueSupplyM a -> UniqueSupply -> (a, UniqueSupply)
runUniqueSupplyM m us = runIdentity $ runUniqueSupplyT m us
...
-- | Get a UniqueSupply within the UniqueSupplyT monad transform
getUs :: Monad m => UniqueSupplyT m UniqueSupply
getUs = UST splitUniqueSupply

-- | Get a Unique within the UniqueSupplyT monad transform
getUniqueUs :: Monad m => UniqueSupplyT m Unique
getUniqueUs = UST takeUniqueFromSupply
