...
b_and  = \x -> if' x id (const False)

b_or :: Bool -> Bool -> Bool -- binary or ||
b_or  = \x -> if' x (const True) id
...
until :: forall (a :: *).
         (a -> Bool) -> (a -> a) -> a -> a
until  = \c f x -> if' (c x) x (until c f (f x))
...
pure :: forall (f :: * -> *).
        ...
        Applicative f =>
        a -> f a
pure  = fst classMethodsApplicative
...
seq_apply :: forall (f :: * -> *). -- m1 <*> m2
             ...
             Applicative f =>
             f (a -> b) -> f a -> f b
seq_apply  = snd classMethodsApplicative
...
seq_applyr :: forall (f :: * -> *). -- m1 <*> m2
              ...
              Applicative f =>
              f a -> f b -> f b
seq_applyr  = compose seq_apply (fmapl id)
...
seq_applyl :: forall (f :: * -> *). -- m1 <*> m2
              ...
              Applicative f =>
              f a -> f b -> f a
seq_applyl  = compose seq_apply (fmap const)
...
foldl :: forall (t :: * -> *).
         ...
         Foldable t =>
         (b -> a -> b) -> b -> t a -> b
foldl  = \f b t -> foldr (\a ctx -> (\x -> ctx (f x a))) id t b

and :: forall (t :: * -> *).
       Foldable t =>
       t Bool -> Bool
and  = foldr b_and True
...
any :: forall (t :: * -> *).
       ...
       Foldable t =>
       (a -> Bool) -> t a -> Bool
any  = \f -> foldr (\x -> if' (f x) True) False

all :: forall (t :: * -> *).
       forall (a :: *).
       Foldable t =>
       (a -> Bool) -> t a -> Bool
all  = \f -> foldr (\x -> flip (if' (f x)) False) True
...
scanl  = \f b xs -> Cons b (if' (null xs) Nil (scanl f (f b (head xs)) (tail xs)))

scanr :: forall (a :: *).
         forall (b :: *).
         (a -> b -> b) -> b -> List a -> List b
scanr  = \f b xs -> let qs = scanr f b (tail xs)
...
cycle :: forall (a :: *).
         List a -> List a
cycle  = \xs -> let ys = append xs ys in ys

dropWhile :: forall (a :: *).
             (a -> Bool) -> List a -> List a
dropWhile  = \f xs -> case xs of
                        Cons y ys -> if' (f y) (dropWhile f ys) xs
                        Nil       -> Nil
...
zip  = \xs ys -> if' (b_or (null xs) (null ys))
                     (Nil)
...
isLT :: forall (a :: *).
        Ord a =>
        a -> a -> Bool
isLT  = \x y -> equals (compare x y) LT

isLE :: forall (a :: *).
        Ord a =>
        a -> a -> Bool
isLE  = \x y -> notEquals (compare x y) GT

isGT :: forall (a :: *).
        Ord a =>
        a -> a -> Bool
isGT  = \x y -> equals (compare x y) GT

isGE :: forall (a :: *).
        Ord a =>
        a -> a -> Bool
isGE  = \x y -> notEquals (compare x y) LT
...
-- Integer
data Digit = D0 | D1 | D2 | D3 | D4 | D5 | D6 | D7 | D8 | D9 deriving Show -- decimal digits

digitCompare :: Digit -> Digit -> Ordering
digitCompare  = \x y -> case x of
                           D0 -> case y of
                                   D0 -> EQ
                                   D1 -> LT
                                   D2 -> LT
...
data Integer = Integer Bool (List Digit) deriving Show -- sign and digits

natural :: List Digit -> Integer -- sign (True = positive) and digits (list of igits in decreasing significance, the way you would write them normally)
natural  = Integer True

digit :: Digit -> Integer -- converts a single igit to a number
digit  = compose natural pure
...
prependToUnifyLengths  = \a b as bs -> if' (null as)
                                           (if' (null bs)
                                                (Pair Nil Nil)
...
addDigit  = \x y -> if' (equals (compare x y) GT)
                        (addDigit y x)
                        (case x of
                           D0 -> Pair D0 y
                           D1 -> case y of
...
digitComplement :: Digit -> Digit
digitComplement  = \x -> case x of
                           D0 -> D0
                           D1 -> D9
                           D2 -> D8
                           D3 -> D7
...
subDigits  = let f = \xs ys -> if' (null xs)
                                   (Cons D0 Nil)
                                   (let zs = f (tail xs) (tail ys)
